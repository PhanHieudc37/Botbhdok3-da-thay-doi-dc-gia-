...
class DomainScraper:
    ...
    def init_database(self):
        ...
        conn.commit()
        conn.close()
        print("Đã khởi tạo cơ sở dữ liệu domain.db với bảng theo dõi giá")
    ...
    async def send_price_change_notification(self, price_changes):
        """Gửi thông báo thay đổi giá domain đến Telegram"""
        if not price_changes:
            return
        
        # Tạo tin nhắn thông báo
        message = f"💰 <b>THAY ĐỔI GIÁ DOMAIN!</b>\n\n"
        message += f"📊 Tổng cộng: <b>{len(price_changes)}</b> domain thay đổi giá\n\n"
        
        # Thêm danh sách domain thay đổi giá (giới hạn 10 domain đầu tiên)
        for i, change in enumerate(price_changes[:10], 1):
            ...
            if float(new_price) > float(old_price):
                ...
                direction = "➡️ KHÔNG ĐỔI"
                emoji = "🟡"
            
            message += f"{i}. <code>{name}</code>\n"
            message += f"   {emoji} {direction}: ￥{old_price} → ￥{new_price}\n\n"
        
        if len(price_changes) > 10:
            message += f"\n... và {len(price_changes) - 10} domain khác\n"
    ...
    async def scrape_domains(self, page):
        ...
        try:
            ...
            price_changes = []  # Danh sách domain thay đổi giá
            ...
            for domain in domains_data:
                ...
                cursor.execute("SELECT id, price FROM domains WHERE name = ?", (name,))
                existing_domain = cursor.fetchone()
                
                if not existing_domain:
                    # Domain mới - thêm vào database
                    cursor.execute("INSERT INTO domains (name, price) VALUES (?, ?)", (name, price))
                    saved_count += 1
                    print(f"Đã lưu domain mới: {name} - {price}")
                    ...
                    # Domain đã tồn tại - kiểm tra thay đổi giá
                    old_price = existing_domain[1]
                    if old_price != price:
                        # Cập nhật giá mới
                        cursor.execute("UPDATE domains SET price = ? WHERE name = ?", (price, name))
                        ...
                        print(f"Giá thay đổi: {name} - {old_price} → {price}")
                        
                        # Thêm vào danh sách thay đổi giá
            ...
            if new_domains:
                print("Đang gửi thông báo domain mới...")
                await self.send_domain_notification(new_domains)
            
            # Gửi thông báo Telegram nếu có thay đổi giá
            if price_changes:
                print("Đang gửi thông báo thay đổi giá...")
                await self.send_price_change_notification(price_changes)
    ...
    def get_price_history(self, domain_name=None, limit=20):
        """Lấy lịch sử thay đổi giá domain"""
        try:
            conn = sqlite3.connect(self.db_name)
            cursor = conn.cursor()
            ...
            if domain_name:
                cursor.execute("""
                    SELECT domain_name, old_price, new_price, change_time 
                    FROM price_history 
                    WHERE domain_name = ? 
    ...
    def get_domain_statistics(self):
        ...
        try:
            ...
            # Domain có thay đổi giá nhiều nhất
            cursor.execute("""
                SELECT domain_name, COUNT(*) as change_count 
                FROM price_history 
                GROUP BY domain_name 
                ORDER BY change_count DESC 
                LIMIT 5
            """)
            top_changed = cursor.fetchall()
            ...
            return {
                ...
                'top_changed_domains': top_changed
            }
